...
class FalseBanView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        from cogs.whitelist import is_owner, is_whitelisted
...
class Antinuke(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        # If antinuke is enabled, repeated attempts within the window count
        # toward the mass-action ban threshold too (covers @everyone nuke spam).
        if self.is_enabled(message.guild.id):
            await self.record_and_check(message.guild, message.author, "role/everyone pings")
